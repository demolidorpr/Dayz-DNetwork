...
if (_isPlayer && _unitIsPlayer) then {
	_message = format["%1, stop shooting at friendly players!", _source];
	[nil, nil, rspawn, [_source, _message], { (_this select 0) sideChat (_this select 1) }] call RE;
	if(true) exitWith{};
};
